...
2. 사진 속의 색감, 주요 사물, 전반적인 분위기를 가장 먼저 상세히 분석할 것.
3. 반드시 다음 순서로 작성할 것:
   - 첫 줄: 시 제목
   - 둘째 줄: 시인 이름
   - 셋째 줄부터: 시 본문 (각 행은 \\n 으로 줄바꿈, 연과 연 사이는 반드시 \\n\\n 으로 구분)
4. 시가 너무 길 경우, 첫 연만 보여주고 반드시 "시가 너무 길어 첫 연만 보여드립니다."라는 문구를 시에 대한 설명 앞부분에 반드시 포함할 것.
5. 시 본문이 끝난 후 빈 줄 하나를 추가할 것.
6. 이후 시에 대한 설명, 사진에 보이는 사물, 색감, 분위기를 바탕으로 사용자의 감정에 어울리는 메시지를 자연스럽게 작성할 것.
7. 설명과 시가 절대 섞이지 않도록 분리해서 작성할 것.
8. 링크, 코드블록, 따옴표 등 특수문자는 포함하지 말 것.
9. 사람 관련 정보(얼굴, 나이, 성별 등)는 절대 언급하지 말 것.
10. 반드시 일관된 형식을 유지할 것.
          ...
          content: [
            {
              type: 'text',
              text:
              '이 이미지를 보고 색감, 사물, 분위기를 최대한 구체적이고 상세하게 분석한 후, 가장 어울리는 한국의 시를 추천해줘. ' +
    '색감과 사물은 반드시 명시적으로 분석하고, 사람에 대한 정보는 절대 언급하지 말 것. ' +
