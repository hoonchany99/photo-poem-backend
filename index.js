...
5. 이후 시에 대한 설명을 작성할 것:
   - 반드시 사진 속 "주요 사물"이 시의 주제나 분위기와 어떻게 연결되는지 설명할 것.
   - 사진의 "분위기"와 감정 점수(${emotionScore}/10, 0은 매우 잔잔하고 10은 매우 흥분된 상태)를 바탕으로 시를 선택한 이유를 설명할 것.
   - 이 사진을 올린 사람의 심정을 추측하고, 감정적으로 도움이 될 만한 따뜻한 말을 자연스럽게 덧붙일 것.
6. 설명과 시가 절대 섞이지 않도록 반드시 빈 줄로 구분할 것.
...
시가 너무 길어 첫 연만 보여드립니다. 이 시는 하늘, 바람, 잎새와 같은 자연 사물들을 통해 스스로를 성찰하는 내용을 담고 있습니다. 사진에서 보이는 나무와 잔잔한 호수, 그리고 감정 점수 2/10의 조용한 분위기가 이 시의 메시지와 잘 어울립니다. 사진을 올린 당신은 아마도 평온함을 찾고 싶었던 것 같아요. 오늘 하루도 충분히 잘하고 있으니, 스스로를 다정하게 바라봐 주세요.
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        `사용자의 감정 점수는 ${emotionScore}이며, 0은 매우 잔잔한 상태, 10은 매우 흥분된 상태를 의미한다. 이를 반드시 고려할 것.`,
            },
